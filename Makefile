...
EXTRA_CFLAGS = -Wall -D_U_= $(WIRESHARK_FLAGS) `pkg-config --cflags glib-2.0` -fPIC -DPIC

WIRESHARK_LDFLAGS = -L$(WIRESHARK_LIBRARY_PATH) -lwireshark
EXTRA_LDFLAGS = -Wl,--export-dynamic $(WIRESHARK_LDFLAGS) `pkg-config --libs glib-2.0`
...
%.o : %.c
	$(Q_CC)$(CC) -c $(CFLAGS) $(EXTRA_CFLAGS) $< -o $@
